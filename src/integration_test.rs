...
#[test]
// receive tokens and release upon approval
fn test_instantiate() {
    let mut router = mock_app();

    const TOKEN1_DENOM: &str = "token1";
    const TOKEN2_DENOM: &str = "token2";

    let owner = Addr::unchecked("owner");
    let funds = coins(2000, TOKEN1_DENOM);
    ...
    assert_eq!(info.lp_token_denom, "contract2".to_string());
    ...
    let amm_id = router.store_code(contract_amm());
    let msg = InstantiateMsg {
        token1_denom: TOKEN1_DENOM.into(),
        token2_denom: TOKEN2_DENOM.into(),
        lp_token_code_id: cw20_id,
        owner: Some(owner.to_string()),
        lp_fee_percent,
        protocol_fee_percent,
